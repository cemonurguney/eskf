...
% Senaryo ayarli truth ve sensor verisi uretir.
%
% Quaternion / gyro konvansiyonu:
%   - q_nb    : body -> nav
%   - IMU gyro: body-frame angular velocity
%   - truth gyro uretiminde: dq_b = q_{k-1}^* otimes q_k
%
...
if enable_wind_tas
    % Fixed-wing-like truth trajectory icinde wind kullandiysa ayni wind'i kullan.
    % Legacy senaryoda burada wind uret.
...
% Daha mantikli fixed-wing sentetik truth uretir.
%
% Frame convention:
%   NED frame: p = [N; E; D], D positive down
%   q_nb: body -> nav
%
% Tasarim mantigi:
%   - TAS 11-18 m/s bandinda kalir, 4 m/s gibi ucak-olmayan sacmaliklara dusmez.
%   - Heading-rate schedule ile loiter/racetrack/S-turn benzeri manevralar uretir.
%   - Roll, coordinated-turn denkleminden turetilir: phi = atan(Va_h * psi_dot / g).
%   - Pitch, flight-path angle gamma ile tutarlidir.
%   - Wind yavas degisen ve gust benzeri kisimlar iceren ama log dosyasini kopyalamayan bir profildir.
...
%% ---------------- Airspeed profile ----------------
Va_trim = getfield_with_default_local(fw, 'Va_trim', 14.5);
Va_min  = getfield_with_default_local(fw, 'Va_min', 10.5);
Va_max  = getfield_with_default_local(fw, 'Va_max', 18.5);

Va = Va_trim ...
    + 1.25 * sin(2*pi*t/155 + 0.25) ...
    + 0.65 * sin(2*pi*t/47  + 1.10) ...
    + 0.90 * smooth_gate_local(t,  95, 145, 7) ...
    - 0.70 * smooth_gate_local(t, 250, 290, 8);
Va = min(max(Va, Va_min), Va_max);

%% ---------------- Heading-rate schedule ----------------
yaw0 = deg2rad(getfield_with_default_local(fw, 'yaw0_deg', -80.0));
max_yaw_rate = deg2rad(getfield_with_default_local(fw, 'max_yaw_rate_deg_s', 32.0));

% Base wandering + scheduled turns. Bu, gercek logun kopyasi degil;
% sadece fixed-wing icin makul manevra ve observability uretir.
yaw_rate = deg2rad(2.8) * sin(2*pi*t/115 + 0.4) ...
         + deg2rad(1.4) * sin(2*pi*t/42  + 1.7);

yaw_rate = yaw_rate ...
    + deg2rad( 25.0) * smooth_gate_local(t,  25,  72, 4) ...
    + deg2rad(-27.0) * smooth_gate_local(t,  88, 135, 4) ...
    + deg2rad( 23.0) * smooth_gate_local(t, 158, 205, 5) ...
    + deg2rad(-24.0) * smooth_gate_local(t, 225, 276, 5) ...
    + deg2rad( 19.0) * smooth_gate_local(t, 302, 350, 6);

yaw_rate = smooth_limit_local(yaw_rate, max_yaw_rate);
yaw_true = yaw0 + cumtrapz(t, yaw_rate);

%% ---------------- Climb / descent profile ----------------
max_gamma = deg2rad(getfield_with_default_local(fw, 'max_gamma_deg', 8.0));

gamma = deg2rad(3.0) * sin(2*pi*t/92 + 0.7) ...
      + deg2rad(1.5) * sin(2*pi*t/38 + 2.2) ...
      + deg2rad(2.2) * smooth_gate_local(t,  40,  92, 8) ...
      - deg2rad(2.0) * smooth_gate_local(t, 210, 248, 8);
gamma = smooth_limit_local(gamma, max_gamma);

%% ---------------- Wind profile ----------------
% Defaultlar gercek dosyadaki olcege yakin, ama ayni profil degil.
...
wN = w_base(1) ...
   + 0.40 * sin(2*pi*t/190 + 0.1) ...
   + 0.18 * sin(2*pi*t/57  + 1.8) ...
   + 0.25 * smooth_gate_local(t, 120, 170, 15);

wE = w_base(2) ...
   + 0.35 * cos(2*pi*t/175 + 0.6) ...
   + 0.16 * sin(2*pi*t/49  + 0.5) ...
   - 0.20 * smooth_gate_local(t, 260, 325, 18);

wind_ref = [wN; wE];

%% ---------------- Air-relative velocity -> ground velocity ----------------
Va_h = Va .* cos(gamma);
v_air_N = Va_h .* cos(yaw_true);
v_air_E = Va_h .* sin(yaw_true);
v_air_D = -Va .* sin(gamma);   % gamma > 0 means climb, D velocity negative
...
%% ---------------- Coordinated-turn attitude ----------------
g_abs = abs(params.g_n(3));
max_roll = deg2rad(getfield_with_default_local(fw, 'max_roll_deg', 45.0));

roll_true = atan2(Va_h .* yaw_rate, g_abs);
roll_true = min(max(roll_true, -max_roll), max_roll);

pitch_true = gamma ...
    + deg2rad(1.2) * sin(2*pi*t/60 + 0.3) ...
    + deg2rad(0.5) * sin(2*pi*t/21);
pitch_true = smooth_limit_local(pitch_true, deg2rad(12.0));
...
%% ---------------- Body-frame velocity / acceleration command ----------------
v_body = zeros(3, N);
...
fprintf('[simulate] Fixed-wing loiter/racetrack truth enabled.\n');
...
fprintf('[simulate] Pitch mean/min/max [deg]: %.3f / %.3f / %.3f\n', ...
    mean(rad2deg(pitch_true)), min(rad2deg(pitch_true)), max(rad2deg(pitch_true)));
end

%% =============================================================
% ATTITUDE SENARYO URETICISI, LEGACY
% =============================================================
...
switch mode
    ...
    case 'piecewise'
        ...
        if size(E,1) ~= 3
            error('attitude.piecewise.euler_deg boyutu 3xM olmalidir.');
        end
...
%% =============================================================
% TRANSLASYON SENARYO URETICISI, LEGACY
% =============================================================
...
switch mode
    ...
    case 'piecewise_accel'
        ...
        if size(A,1) ~= 3
            error('translation.piecewise.acc_body boyutu 3xM olmalidir.');
        end
        if size(A,2) ~= numel(tb)-1
            error('translation.piecewise icin size(acc_body,2) = numel(t_breaks)-1 olmalidir.');
        end
...
function y = smooth_gate_local(t, t1, t2, tau)
% 0 -> 1 -> 0 smooth rectangular gate.
y = 0.5 * (tanh((t - t1)./tau) - tanh((t - t2)./tau));
end

function y = smooth_limit_local(x, limit_abs)
% Differentiable saturation. Sert clamp yerine daha temiz gyro/acc uretir.
y = limit_abs * tanh(x ./ limit_abs);
end
